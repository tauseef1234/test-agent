...
properly (e.g. [c:HAS_MORTGAGE] instead of [:HAS_MORTGAGE]). Do not run any
queries that would add to or delete from
the database. Make sure to alias all statements that follow as with
statement (e.g. WITH c as customer, m.loan_amount as loan_amount)
...
{example_queries}
Warning:
- Never return a product FAQs about Mortgage node without explicitly returning all of the properties
...
follow as with statement (e.g. WITH c as customer, m.loan_amount as
loan_amount)
- If you need to divide numbers, make sure to filter the denominator to be non
zero.

String category values:
# Based on the new graph model, add relevant string categories if known. For example:
# Loan statuses are one of: 'Active', 'Closed'
# Fee types could be: 'Late Fee', 'Processing Fee'
# This section should be populated based on the actual data types and values in your Neo4j instance.
...
When names are provided in the query results, such as customer names or loan numbers,
beware of any names that have commas or other punctuation in them. Make sure you return any list of names in a
...
bank_cypher_chain = GraphCypherQAChain.from_llm(
